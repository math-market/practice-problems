#!/usr/bin/env python3
"""De Bruijn sequence B(2,6): a cyclic binary sequence of length 64
in which every 6-bit word occurs exactly once.
...
if len(seq) != K**N: no("sequence has length %d; a B(2,6) sequence has length %d" % (len(seq), K**N))
if any(x not in (0, 1) for x in seq): bad("entries must be 0 or 1")
wins = [tuple(seq[(i+j) % len(seq)] for j in range(N)) for i in range(len(seq))]
if len(set(wins)) != K**N:
    dup = len(wins) - len(set(wins))
    no("%d of the %d cyclic 6-windows are repeats; every window must occur exactly once" % (dup, len(wins)))
yes("cyclic binary sequence of length 64 containing all 64 six-bit words exactly once")
