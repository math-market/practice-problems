#!/usr/bin/env python3
"""Steiner triple system on 15 points: 35 triples from 0..14 with
every pair in exactly one triple.
...
for t in T:
    ...
    if any(not isinstance(x, int) or not 0 <= x < V for x in t): bad("points must be integers in 0..14")
if len(T) != NEED: no("an STS(15) has %d triples; got %d" % (NEED, len(T)))
for t in T:
...
yes("Steiner triple system on 15 points")
