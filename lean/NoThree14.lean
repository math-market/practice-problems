...
condition failed, append these lines and re-run `lean NoThree14.lean`:

    def P : List (List Int) := [your, twenty, eight, points]
    #eval P.length == 28
    #eval P.all (fun p => p.length == 2 && p.all (fun x => 0 <= x && x < 14))
    #eval (List.range 28).all (fun i => (List.range 28).all (fun j => (List.range 28).all (fun k =>
...
/-- 28 distinct grid points, no three collinear. -/
def nothree14OK (P : List (List Int)) : Bool :=
  P.length == 28 && P.all (fun p => p.length == 2 && p.all (fun x => 0 <= x && x < 14)) &&
  (List.range 28).all (fun i => (List.range 28).all (fun j =>
     if i < j then P.getD i [] != P.getD j [] else true)) &&
  (List.range 28).all (fun i => (List.range 28).all (fun j => (List.range 28).all (fun k =>
     if i < j && j < k then !(col3 (P.getD i []) (P.getD j []) (P.getD k [])) else true)))

/-- The board. -/
theorem nothree14 : ∃ P : List (List Int), nothree14OK P = true := by
  sorry
