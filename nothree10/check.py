#!/usr/bin/env python3
"""No-three-in-line on a 10x10 grid: 20 points, no three collinear.
...
for p in P:
    ...
    if any(not isinstance(x, int) or not 0 <= x < N for x in p): bad("coordinates must be integers 0..9")
...
yes("20 points on the 10x10 grid with no three collinear")
